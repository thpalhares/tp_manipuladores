...
for shape in shapes:
    ...
    # get the right pen
    points_htm.append(pens_htm[i*3+k] * ub.Utils.roty(np.pi) * ub.Utils.trn([0, 0, 0.025]))
...
while point_index < len(points_htm):
    ...
    if has_pen == False and has_drawn == False:
        dist_pen = [fk[0:3, 3] - pens_htm[i*3+k][0:3, 3]]
        if np.linalg.norm(dist_pen) < 0.03:
            robot.attach_object(pens[i*3+k])
            has_pen = True
